...
void Highlife::CalculateGrid(Grid* _grid)
{
    ...
    for (size_t i = 0; i < matrix.size(); ++i)
    {
        for (size_t j = 0; j < matrix[i].size(); ++j)
        {
            ...
            // Check if the cell should live with 2 or 
            if (matrix[i][j] == 1 && (liveNeighbors != 2 && liveNeighbors != 3))
            {
                newMatrix[i][j] = 0;
            }
            else if (matrix[i][j] == 0 && (liveNeighbors == 3 || liveNeighbors == 6))
            {
                newMatrix[i][j] = 1;
            }
        }
